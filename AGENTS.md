# AGENTS.md — 名探偵コナンTCG Webアプリ プロジェクト規約

このファイルは Codex がこのプロジェクトで作業する際の規約を定める。
...
ユーザーレビュー(RV)を依頼する **前** に、Codex自身が必ず以下を実施：
...
- [ ] 推測でルールを補完しない（AGENTS.md「ルール参照義務」に従う）
...
- **基本作業は opus 主体**: 通常作業 (カード実装/調査/ドキュメント等) のオーケストレーションと
  実作業は `model:'opus'` を明示。
- **難判断も当面 opus を最初から使う (fable は使わない)** (2026-06-14 改定): `Codex-fable-5` が
...
1. `.claude/AGENTS.md` — 本ファイル (規約 / 運用ルール)
...
| **規約・手順** | `AGENTS.md` (本ファイル) | 運用ルール変更時 | 手書き |

新しいセッションの Codex が `README.md` → `CHANGELOG.md` → `.claude/auto/structure.md` の 3 ファイルだけで「何をやっているプロジェクトか」「最新の状況」「どこに何があるか」を把握できる状態を保つこと。
