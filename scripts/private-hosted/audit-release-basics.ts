...
const DESTINATION = /(?:(?:https?|wss?):)?\/\/(?:[^\s"'`<>\\(){}@/:;]+(?::[^\s"'`<>\\(){}@/;]*)?@)?(?:\[[0-9A-Fa-f:.]+\]|localhost|[A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)+)(?::\d+)?(?:\/[^\s"'`<>\\(){}\];]*)?/g;
const SECRET_MARKERS: Array<[RegExp, string]> = [
  [/-----BEGIN(?: [A-Z]+)? PRIVATE KEY-----/i, "private key"],
  [/(?:Authorization|authorization)["']?\s*[:=]\s*["']Bearer\s+[^\s"'\\]{16,}/i, "bearer token"],
  [/(?:Authorization|authorization)["']?\s*[:=]\s*["']Basic\s+[A-Za-z0-9+/=]{16,}/i, "basic credential"],
  [/(?:CLOUDFLARE_API_TOKEN|CF_API_TOKEN|access_token|api_token|auth_token)["']?\s*[:=]\s*["']?[^\s"'&#\r\n]{16,}/i, "API token"],
  [/(?:CF-Access-Client-Secret|CF_ACCESS_CLIENT_SECRET)["']?\s*[:=]\s*["']?[^\s"'&#,}\r\n]{16,}/i, "Cloudflare Access client secret"],
...
function artifactText(bytes: Buffer): string {
  if (
    ...
    throw new Error("basic release scan rejected: UTF-32 artifact is unsupported");
  }
...
async function releaseTextFiles(
  ...
  const label = artifactRoot === resolve(repositoryRoot, "dist")
    ? "dist"
  ...
  if (!artifactStat.isDirectory() || artifactStat.isSymbolicLink()) {
    throw new Error(`basic release scan rejected: ${label} must be a regular directory`);
  }
  ...
  const visit = async (directory: string): Promise<void> => {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) {
        throw new Error(`basic release scan rejected: symbolic link: ${displayPath(label, artifactRoot, path)}`);
      }
...
export async function scanReleaseSecrets(
  ...
  for (const { file, source } of await releaseTextFiles(resolve(root), artifactDirectory)) {
    const normalized = normalizedLiteralText(source);
    for (const [marker, label] of SECRET_MARKERS) {
      if (marker.test(normalized)) findings.push({ file, code: "embedded-secret", detail: label });
    }
  }
  return findings;
}
...
function approvedDestination(value: string): boolean {
  ...
  return APPROVED_EXACT_DESTINATIONS.has(withoutQuery) ||
    APPROVED_DESTINATION_PREFIXES.some((allowed) => withoutQuery.startsWith(allowed));
}
...
export async function scanReleaseDestinations(
  ...
  const findings: ReleaseBasicFinding[] = [];
  for (const { file, source } of await releaseTextFiles(resolve(root), artifactDirectory)) {
    const destinations = [...new Set(normalizedLiteralText(source).match(DESTINATION) ?? [])].sort();
    for (const destination of destinations) {
      if (!approvedDestination(destination)) {
        findings.push({ file, code: "external-destination", detail: destinationDetail(destination) });
      }
    }
  }
  return findings;
...
async function cli(): Promise<void> {
  ...
  if (process.argv.length !== 3 || !["secrets", "destinations"].includes(check ?? "")) {
  ...
  const artifactDirectory = configuredArtifact ?? resolve(MODULE_REPOSITORY_ROOT, "dist");
  const findings = check === "secrets"
    ? await scanReleaseSecrets(MODULE_REPOSITORY_ROOT, artifactDirectory)
    : await scanReleaseDestinations(MODULE_REPOSITORY_ROOT, artifactDirectory);
  process.stdout.write(`${JSON.stringify({ schemaVersion: 1, ok: findings.length === 0, findings }, null, 2)}\n`);
