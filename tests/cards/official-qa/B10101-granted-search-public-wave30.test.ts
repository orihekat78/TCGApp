...
// qa: card:B10101:e1cf102a1d6d73ee31b4ce9b24f69c56d6a3d4d28d31ab0ad025085254877f2b

import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { B10101, B10101P } from '@/cards/ct-p10/B10101';
...
const SHORT_QA = 'card:B10101:e1cf102a1d6d73ee31b4ce9b24f69c56d6a3d4d28d31ab0ad025085254877f2b';
...
const REFRESH = 'B10101_PUBLIC_REFRESH';
...
const REFRESH_DEF = character(REFRESH);
...
function stateFor(player: Player, options: {
  caseId?: string;
  deck?: string[];
  remove?: string[];
} = {}): GameState {
  ...
  state.players[player].case = {
    cardId: options.caseId ?? B10101.id, status: '解決編', requiredEvidence: 7,
  ...
  state.players[player].deck = options.deck ? [...options.deck] : [FILLER, ASSAULT, DECOY, TAIL];
  state.players[player].remove = options.remove ? [...options.remove] : [];
  state.players[defender].deck = [FILLER, DECOY, TAIL, FILLER];
  return state;
}

function install(player: Player, options: Parameters<typeof stateFor>[1] = {}): void {
  expect(useGameStateStore.getState().setGameState(stateFor(player, options))).toBe(true);
}

function grantThroughDeclaredAbility(player: Player, caseId = B10101.id): void {
  ...
  if (player === 'self') {
    ...
    expect(grant?.source).toMatchObject({ cardId: caseId, abilityId: 'a2', uid: `case:${player}`, area: 'case' });
...
beforeEach(() => {
  ...
  _resetRegistry();
  for (const card of [B10101, B10101P, GIVER_DEF, VICTIM_DEF, ASSAULT_DEF, FILLER_DEF, DECOY_DEF, TAIL_DEF, REFRESH_DEF]) {
    register(card);
  }
...
describe('B10101 public declared-grant and contact-removal path', () => {
  ...
  it(`${SHORT_QA}: refreshes only after selecting the sole deck card for both physical case printings`, () => {
    const proven: string[] = [];
    for (const caseId of [B10101.id, B10101P.id]) {
      useGameStateStore.getState().resetMatchSessionState();
      install('self', { caseId, deck: [ASSAULT], remove: [REFRESH] });
      grantThroughDeclaredAbility('self', caseId);
      const actionId = contactAndJudge('self');

      const pick = useGameStateStore.getState().pendingEffectPick;
      expect(pick).toMatchObject({
        player: 'self', atomVerb: 'deckRevealUntil', nMin: 0, nMax: 1,
        source: { cardId: GIVER, abilityId: 'b10101-granted-assault-search', uid: 'giver', area: 'scene' },
      });
      expect(pick?.candidates.map(candidate => candidate.cardId)).toEqual([ASSAULT]);
      expect(useGameStateStore.getState().pendingDeckReveal).toMatchObject({
        player: 'self', visibility: 'private', viewer: 'self', revealed: [ASSAULT], awaitingPick: true,
      });
      expect(current().players.self.deck, `${caseId}: viewed occurrence remains in deck`).toEqual([ASSAULT]);
      expect(current().refreshCount.self, `${caseId}: look does not refresh`).toBe(0);
      ...
      expect(publication).toMatchObject({
        owner: 'self', audience: 'all', cardIds: [ASSAULT], lifetime: 'presentation',
        origin: 'deck-selected-card', source: { cardId: GIVER, abilityId: 'b10101-granted-assault-search', uid: 'giver' },
      });
      expect(publication?.cardIds).toEqual([ASSAULT]);
      expect(current().players.self.hand).toEqual([ASSAULT]);
    ...
    expect(proven).toEqual([B10101.id, B10101P.id]);
  });
