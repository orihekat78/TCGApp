...
import {
  SAMPLE_DECK,
  SAMPLE_DECK_OPP,
} from "../../meta-app/src/data/sampleDeck";
...
describe("HOME deck identity", () => {
  ...
  it("renders the required header order with the supplied brand logo and a single new-game entry", () => {
    ...
    expect(
      container.querySelector('img[alt="DETECTIVE CONAN"]'),
    ).not.toBeNull();
    expect(nav!.querySelectorAll('svg[aria-hidden="true"]')).toHaveLength(7);
    expect(
      nav!
        .querySelector('button[data-route="setup"]')
        ?.classList.contains("home-nav-start"),
    ).toBe(true);
    expect(
      Array.from(container.querySelectorAll("button")).filter(
        (button) => button.textContent?.trim() === "ゲーム開始",
      ),
    ).toHaveLength(1);
  });

  it("shows the required private fan-project notice on the initial HOME screen", () => {
    act(() => root.render(<HomeScreen onNav={() => undefined} />));

    const notice = container.querySelector('[data-testid="home-legal-notice"]');
    expect(notice).not.toBeNull();
    expect(notice?.textContent).toContain(
      "This is an unofficial fan project for personal use only.",
    );
    expect(notice?.textContent).toContain("© 青山剛昌／小学館 © TOMY");
    expect(notice?.textContent).toContain("公式商品ではありません。");
  });
  ...
  it("makes deck name the only H1 and shows partner plus incident identity without redundant labels", () => {
    ...
    expect(
      Array.from(container.querySelectorAll("h1")).map(
        (node) => node.textContent,
      ),
    ).toEqual(["少年探偵団・標準"]);
    expect(
      container.querySelector(
        'figure[aria-labelledby="home-card-D08001-name"]',
      ),
    ).not.toBeNull();
    expect(
      container.querySelector(
        'figure[aria-labelledby="home-card-D08026-name"]',
      ),
    ).not.toBeNull();
  ...
  it("opens the HOME-owned deck selector without navigating away and keeps official NEWS links external", () => {
    ...
    const gameStart = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.trim() === "ゲーム開始",
    );
    const changeDeck = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.trim() === "使用デッキを変更",
    );
    act(() => gameStart?.click());
    ...
    const newsLink =
      container.querySelector<HTMLAnchorElement>(".home-news-list a");
    expect(newsLink?.href).toBe(
      "https://www.takaratomy.co.jp/products/conan-cardgame/news/20260731.html",
    );
  ...
  it("keeps a deck choice provisional until confirmation", () => {
    ...
    const choices = container.querySelectorAll<HTMLInputElement>(
      'input[name="home-active-deck"]',
    );
    expect(choices).toHaveLength(2);
    act(() => choices[1]!.click());
    expect(useDecksStore.getState().activeDeckId).toBe(SAMPLE_DECK.id);
    expect(container.querySelector("#home-deck-name")?.textContent).toBe(
      SAMPLE_DECK.name,
    );
  ...
  it("discards a provisional choice on Escape without stealing a later focus move", async () => {
    ...
    const nextAction = container.querySelector<HTMLButtonElement>(
      'button[data-route="deck"]',
    )!;
    nextAction.focus();
    await act(
      async () =>
  ...
  it("keeps an unplayable saved deck visible but unavailable for selection", () => {
    ...
    const invalidChoice = container.querySelector<HTMLInputElement>(
      'input[value="needs-adjustment"]',
    );
  ...
  it("distinguishes NEWS loading from an unavailable result and keeps the official recovery link", () => {
    ...
    expect(
      container.querySelector<HTMLAnchorElement>(
        'a[href="https://www.takaratomy.co.jp/products/conan-cardgame/"]',
      ),
    ).not.toBeNull();
  });
...
describe("HOME compact landscape accessibility", () => {
  it("keeps heading actions at the 44px touch target minimum", () => {
    ...
    expect(css).toMatch(
      /@media \(max-width:\s*900px\) and \(orientation:\s*landscape\)[\s\S]*\.home-section-heading a,[\s\S]*\.home-section-heading button\s*\{[^}]*min-height:\s*44px;/,
    );
  });
});
