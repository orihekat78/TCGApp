...
describe('official NEWS boundary', () => {
  ...
  it('returns an empty list for missing or malformed listing markup', () => {
    expect(parseOfficialNews('<main><a href="/products/conan-cardgame/news/x">no fields</a></main>')).toEqual([]);
  });
  ...
  it('uses a fresh six-hour cache without issuing a second request', async () => {
    ...
    const first = await loadOfficialNews({ fetcher, storage: localStorage, now });
    const second = await loadOfficialNews({ fetcher, storage: localStorage, now: now + 6 * 60 * 60 * 1000 - 1 });

    expect(first.source).toBe('network');
    expect(second.source).toBe('cache');
    expect(second.items).toEqual(first.items);
    expect(fetcher).toHaveBeenCalledTimes(1);
    expect(fetcher).toHaveBeenCalledWith(OFFICIAL_NEWS_URL, expect.objectContaining({ signal: expect.any(AbortSignal) }));
  });

  it('refreshes a fresh legacy cache created under the three-item limit', async () => {
    const now = Date.UTC(2026, 7, 3, 12);
    localStorage.setItem(OFFICIAL_NEWS_CACHE_KEY, JSON.stringify({
      version: 1,
      fetchedAt: now,
    ...
    const result = await loadOfficialNews({ fetcher, storage: localStorage, now });

    expect(result.source).toBe('network');
    expect(result.items).toEqual(parseOfficialNews(HTML));
    expect(fetcher).toHaveBeenCalledTimes(1);
    expect(JSON.parse(localStorage.getItem(OFFICIAL_NEWS_CACHE_KEY)!)).toMatchObject({ version: 2 });
  });
  ...
  it('rejects malformed cached identities and overlong metadata', () => {
    ...
    expect(readOfficialNewsCache({ storage: localStorage, now: Date.UTC(2026, 7, 2, 12) })).toBeNull();
  });
  ...
  it('exposes stale cached metadata synchronously for stale-while-revalidate rendering', async () => {
    ...
    const cached = readOfficialNewsCache({ storage: localStorage, now: now + 6 * 60 * 60 * 1000 });

    expect(cached).toMatchObject({ source: 'stale', items: parseOfficialNews(HTML) });
  });
  ...
  it('does not start a request when cancellation already happened', async () => {
    ...
    await expect(loadOfficialNews({ fetcher, storage: localStorage, signal: controller.signal }))
      .rejects.toMatchObject({ name: 'AbortError' });
    expect(fetcher).not.toHaveBeenCalled();
  });

  it('rejects impossible calendar dates from remote markup', () => {
    const html = HTML.replaceAll('2026-07-31', '2026-99-99').replaceAll('2026.7.31', '2026.99.99');
    expect(parseOfficialNews(html).map((item) => item.title)).not.toContain('【7月31日更新】探偵サミット2026');
  });
});
