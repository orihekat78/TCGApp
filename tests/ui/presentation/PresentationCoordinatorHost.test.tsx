...
describe('PresentationCoordinatorHost', () => {
  ...
  it('uses one horizontal strip anchored inside the centered playmat at 851x393', () => {
    ...
    expect(Number.parseFloat(host?.style.getPropertyValue('--presentation-strip-left') ?? ''))
      .toBeCloseTo(84.1667, 3);
    expect(Number.parseFloat(host?.style.getPropertyValue('--presentation-strip-width') ?? ''))
