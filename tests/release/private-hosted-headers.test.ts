...
const EXPECTED = `/*
  Content-Security-Policy: default-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://www.takaratomy.co.jp; font-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'none'; worker-src 'none'
...
function cardImageStems(): Set<string> {
  const source = readdirSync("src/cards", { recursive: true, encoding: "utf8" })
    ...
    .map((path) =>
      readFileSync(`src/cards/${path.replaceAll("\\", "/")}`, "utf8"),
    )
    .join("\n");
  return new Set(
    [...source.matchAll(/\bimageUrl:\s*['"]([^'"]+)['"]/g)].map(
...
describe("private hosted security headers", () => {
  ...
  it("pins the release toolchain and ignores Wrangler state", () => {
    ...
    expect(pkg.scripts?.["build:meta"]).toBe(
      "vite build --config vite.config.private-hosted.ts",
    );
  ...
  it("keeps Node and removed image helpers outside the browser boundary", () => {
    ...
    const assetNames = readdirSync("dist/assets").filter((name) =>
      name.endsWith(".js"),
    );
    expect(assetNames.length).toBeGreaterThan(0);
    expect(assetNames.some((name) => name.includes("tsv-loader-fs"))).toBe(
      false,
    );
  ...
  it("ships the static Meta artifact with only the reviewed API route", () => {
    ...
    expect(
      isCardImagePayload("assets/1743743100639068-build.svg", knownStems),
    ).toBe(true);
    expect(isCardImagePayload("assets/ui-background.webp", knownStems)).toBe(
      false,
    );
    expect(
    ...
    expect(
      files.some((path) =>
        /(?:^|\/)(?:functions|_worker\.js)(?:\/|$)/i.test(path),
      ),
    ).toBe(false);
