...
//   - その中からレベル4以下の【黄】のキャラを1枚まで… => deckRevealUntil filter {color:'黄',levelMax:4,kind:'character'} → conditional bound$matched [deckRevealUntil predicate path honors cardId/color/trait/ap/lp/level/kind (capability-map §deckRevealUntil note, BUG-117/118 fixed). color/levelMax/kind all honored. 1枚まで = single bindMatch + bound:'matched' guard (0-pick legal). Exemplar D01012.ts a1 (青 twin, levelMax:4,kind:'character').]
...
//   - 残りを好きな順番でデッキの下に移す => atom deckToBottomBound {player:'self',bindKey:'$revealed'} [deckToBottomBound verb moves bound cardIds to deck bottom (capability-map §D); exemplar D01012.ts a1 + B01013.ts a1. (engine preserves peek order; 「好きな順番」 player-reorder not surfaced — see notes.)]
