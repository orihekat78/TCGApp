// cards/pr-01/PR240 毛利蘭 (character) — M2 latter batch (charSetCard faceUp honor + area 配列 union + setcard:leave→handAddFromRemove, 2026-07-10)
// PR234 と印字完全同文 (id/no/imageUrl のみ差) — 同一 DSL clone。
...
//     置かれたときに発動する。
//
// 句マッピング (grounding dossier .tmp/_ground/PR240.md):
    ...
    steps: [
      ...
      // 手札かリムーブエリアの〚カード名[シャッフルロマンス]〛イベントを1枚まで選び、host へ表向きセット
      {
        ...
        args: {
          ...
          target: {
            kind: 'pick',
            // area 順 = ['remove','hand'] (edge lens BLOCK 2026-07-10、PR234 と同旨 — 詳細はそちら)。
