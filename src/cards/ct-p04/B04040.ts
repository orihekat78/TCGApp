// cards/ct-p04/B04040 「「Ladies♡ and… Gentlemen!!」」 (イベント) — engine拡張 wave#2 cluster12 (nested-filter-dyn, 2026-06-15)
// rules: 14-refresh.md, 15-abilities-effects.md, 17-icons.md, 20-color-and-switch.md, 26-qa-deck-refresh.md
//
// 公式テキスト:
//   自分のデッキのカードを上から3枚見る。その中から【白】のキャラを1枚まで公開して手札に加え、
//   残りを好きな順番でデッキの下に移す。
...
//   - 上から3枚見る → 【白】のキャラを1枚まで公開して手札に加え => deckRevealUntil{chooseMatch:'upTo', maxN:3, filter:{color:'白',kind:'character'}}
...
//   - 手札から自分のFILEエリアの枚数以下のレベルの【白】のキャラを1枚まで登場 =>
//       sceneEnter{from:'hand', max:1, viaEffect:true, filter:{color:'白', kind:'character', levelMax:{dyn:'$self.fileCount'}}}
...
const a1: AbilityDef = {
  ...
  effect: {
    kind: 'sequence',
    steps: [
      // 上から3枚見る → 【白】のキャラを1枚まで $matched に bind (0枚可)
      { kind: 'atom', verb: 'deckRevealUntil', args: { chooseMatch: 'upTo', player: 'self', filter: { color: '白', kind: 'character' }, maxN: 3, bind: '$revealed', bindMatch: '$matched' } },
      // 該当を選んだ場合 手札に加える
      ...
      // 手札から FILE 枚数以下のレベルの【白】のキャラを1枚まで登場 (cluster12: levelMax dyn)
  ...
  description:
    '自分のデッキのカードを上から3枚見る。その中から【白】のキャラを1枚まで公開して手札に加え、残りを好きな順番でデッキの下に移す。手札から自分のFILEエリアの枚数以下のレベルの【白】のキャラを1枚まで登場させる。',
...
export const B04040: CardDef = {
  id: 'B04040',
  no: '0130/B04040',
  kind: 'event',
  names: ['「Ladies♡ and… Gentlemen!!」'],
  colors: ['白'],
  level: 1,
  traits: [],
  rarity: 'R',
  imageUrl: '1735287759513292.jpg',
  abilities: [a1],
  ruleRefs: [
    'rules/14-refresh.md',
