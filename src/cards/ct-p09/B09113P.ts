...
const a1: AbilityDef = { id: 'a1', type: 'triggered', scope: 'always', trigger: { hook: 'case:to-resolved', selfOnly: true }, effect: { kind: 'atom', verb: 'discard', args: { player: 'self', n: 1 } }, description: 'この事件が解決編になったとき、自分の手札を1枚リムーブする。', ruleRefs: ['rules/15-abilities-effects.md'] };
const a2: AbilityDef = {
  ...
  effect: { kind: 'choice', chooser: 'self', options: [
    { kind: 'conditional', if: { kind: 'scratchTrace', player: 'self', v: '未発見' }, then: { kind: 'sequence', steps: [
...
export const B09113P: CardDef = { id: 'B09113P', no: '1052/B09113P', kind: 'case', names: ['「愛しい愛しい…宿敵さん？」'], colors: ['赤', '黒'], level: 7, caseTraits: [], traits: [], rarity: 'CP', imageUrl: '1775608962404168.jpg', abilities: [a1, a2], ruleRefs: ['rules/15-abilities-effects.md', 'rules/21-declared-ability-cost.md'] };
