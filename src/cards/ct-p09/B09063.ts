...
//   - 相手の現場にレベル7のキャラがいない場合 (no level-7 char in opp scene) => effect-time conditional {kind 'not', c:{kind 'sceneHas', query:{area:'scene', side:'opp', filter:{kind:'character', levelMin:7, levelMax:7}}, nMin:1}}. 公式Q&Aどおり、Lv7がいてdrawできなくても能力自体は発動して【ターン1】を消費するため、ability.conditionには置かない。
//   - カードを1枚引く (mandatory draw 1) => effect-time conditionalのthenに{kind:'atom', verb:'draw', args:{player:'self', n:1}}を置く。draw自体は任意でない。
