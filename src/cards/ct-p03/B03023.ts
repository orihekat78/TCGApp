...
//   - 「相手はデッキ上から1枚公開（その後、元に戻す）」= deckRevealUntil{player:'opp',maxN:1}。
//     この atom は deck を splice せず、公開 window を UI side-channel / effect log に記録する。
//     ※公式Q&A「複数発動しても公開は上から1枚」= 各解決が不変の同じ deck top を読む。
...
const a1: AbilityDef = {
  ...
  // This is a pure public reveal: the card stays at the same deck position.
  effect: {
    kind: 'atom',
    verb: 'deckRevealUntil',
    args: { player: 'opp', maxN: 1, visibility: 'public', viewer: 'all', presentation: 'reveal-return' },
  },
