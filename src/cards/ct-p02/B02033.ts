...
//   - 自分の現場にいるキャラにセットされているカードを合わせて2枚リムーブ => atom charRemoveSetCard {player:'self', side:'self', n:2, filter:{hasSetCards:true}} [src/cards/ct-p07/B07055.ts a1 (同一句「合わせて2枚リムーブ」= charRemoveSetCard n:2、出荷済GREEN) / ct-p07/B07031.ts a2 (同句・known-gap 記録)。engine: scene.ts atomCharRemoveSetCard → PA短縮形 paShortFormAwait → buildShortFormPick(n number→nMin=nMax=2)、apply-pick.ts pickedUids>1 を per-uid sequence 化、mutate/char.ts removeOneSetCard が各キャラから set card 1枚 pop。filter.hasSetCards: candidates.ts:339-342 honor。side:'self'=自分の現場。公式Q&A (event.tsv B02033): 『キャラ2枚に1枚ずつリムーブ可=はい』で per-char 分配を確認。]
