// rules: 03-field-areas.md, 09-cutin-disguise.md, 13-keywords.md, 15-abilities-effects.md, 17-icons.md
import type { AbilityDef, CardDef } from '@/engine/types';
import { partnerColorKeyword } from '../_shared/index.js';

const a1: AbilityDef = {
  id: 'a1', type: 'continuous', scope: 'on-scene',
  condition: { kind: 'and', cs: [{ kind: 'turn', player: 'self' }, { kind: 'sceneHas', query: { area: 'partner-area', side: 'self', filter: { trait: 'ビッグジュエル' } }, nMin: 2 }] },
  continuousModifier: { apDelta: 2000 },
  description: '【自分ターン中】自分のパートナーエリアに〚特徴［ビッグジュエル］〛のカードが2枚以上ある場合、このキャラをAP＋2000する。',
...
const a2: AbilityDef = {
  ...
  effect: { kind: 'atom', verb: 'toPartnerArea', args: { player: 'self', max: 1, filter: { kind: 'event', trait: 'ビッグジュエル' } } },
  description: '【変装時】自分のリムーブエリアにある〚特徴［ビッグジュエル］〛のイベントを1枚まで選び、パートナーエリアに移す。',
