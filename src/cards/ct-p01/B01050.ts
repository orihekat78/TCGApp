...
// 句マッピング:
//   - このキャラはスリープ状態で登場する。 => a1: triggered enter(selfOnly) -> atom sceneSetState {uid:'$self', state:'sleep'} [Exact copy of B01011.ts a1 (src/cards/ct-p01/B01011.ts). Brief 'スリープ状態で登場' pattern. enter hook fires on all entry paths (handUseCard/next-hint/sceneEnter) per capability-map hooks section; official Q&A in ct-p01/character.tsv row 53 confirms '能力や効果によって登場する場合でもスリープ状態で登場 -> はい' so capturing all enter paths via selfOnly enter is correct. sceneSetState verb honored: atom-handlers.ts case 'sceneSetState'.]
