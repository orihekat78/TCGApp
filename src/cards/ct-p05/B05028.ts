...
//   - a1 自分か相手の現場にいるキャラに裏向きでセットされているカードを1枚リムーブしてもよい => chain step1: atom charRemoveSetCard {player:'self', max:1, side:'either', filter:{hasFaceDownSetCards:true}, faceDownOnly:true} [src/cards/ct-p08/B08034.ts a2 と同じ face-down 限定。atom-handlers.ts charRemoveSetCard short-form (uid undefined + player + n/max) builds PA pick via paShortFormAwait; buildShortFormPick honors a.side ?? sideDefault so side:'either' (自分か相手) is honored; max:1/min:0 = 'してもよい' (0-pick skip)。]
