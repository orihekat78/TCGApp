...
export function driveOppTurn(): void {
  ...
  if (store.pendingEffectPick || store.pendingEffectChoice || store.pendingEffectOptional || store.pendingChooseIntercept || store.pendingEffectRepeatOptional || store.pendingDeckReorder || store.pendingDeckPlace) return;
  if (isDriving) return;
  isDriving = true;
  try {
...
export function useOppTurnDriver(): void {
  ...
  useEffect(() => {
    ...
    if (pendingEffectPick || pendingEffectChoice || pendingEffectOptional || pendingChooseIntercept || pendingEffectRepeatOptional || pendingDeckReorder || pendingDeckPlace) return undefined;
    ...
    return undefined;
  }, [turnPlayer, activeActionId, aiSpeedMs, isAiPaused, aiStepCounter, pendingEffectPick, pendingEffectChoice, pendingEffectOptional, pendingChooseIntercept, pendingEffectRepeatOptional, pendingDeckReorder, pendingDeckPlace, oppMoveTick]);
}
