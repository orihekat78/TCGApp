...
type PresentationStripStyle = CSSProperties & {
  '--presentation-strip-left'?: string;
  '--presentation-strip-top'?: string;
  '--presentation-strip-width'?: string;
};
...
export function PresentationCoordinatorHost({
  ...
  const stripStyle: PresentationStripStyle = {
    '--presentation-strip-left': `${viewportLayout.left + ((viewportLayout.renderedWidth - stripWidth) / 2)}px`,
    '--presentation-strip-top': `${viewportLayout.top + 8}px`,
    '--presentation-strip-width': `${stripWidth}px`,
    pointerEvents: 'none',
  };
  ...
  const startSequence = useCallback((runIdentity: string, epoch: number): void => {
    ...
    const schedule = (delay: number, callback: () => void): void => {
      sequenceTimersRef.current.push(setTimeout(callback, delay));
    };
    schedule(causeMs, () => setPhase('target'));
    schedule(causeMs + targetMs, () => setPhase('result'));
    schedule(causeMs + targetMs + resultMs, () => setPhase('settle'));
    schedule(causeMs + targetMs + resultMs + settleMs, () => {
