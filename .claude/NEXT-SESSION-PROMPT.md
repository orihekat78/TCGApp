...
> - **次セッション = W5 (+W6 着手)**:
>   - **W5 dyn/cost (★設計済 `.tmp/_w5_specs.json`、sonnet5 workflow 4/4 unshipped 再確認済・98f7bada 基準)**: r37 removeDeckTop.n dyn 化 (B04088/P、$self.oppSceneCount*2) / r38 evidence-flip-count bind (B08028) / r47 $bound.<key>.level/cardName dyn root (B04074 は P14 併発で句単位) / r43 self-latch (T3、sole=B08059 のみ — **B04069 は再スコープで engine0 判明 = 既存 sceneHas で card-phase authorable**)。
>     synthesis 指示: **step1 = 統合 `resolveBound(state,rest,ctx,orig)` を dyn/eval.ts に1回だけ実装** (r47 field=level/cardName + r38 field=count が共有、シグネチャ衝突回避) → effect.ts 編集は r37→r47→r43 逐次 → r43 は最後に isolate (matchOneFilter levelIn × continuousDeltaSafe 再入の結合テスト要)。
