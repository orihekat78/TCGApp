...
- 出荷済 **1730** / corpus 2074 = **残り未実装 344 printings**。**refuse-1行/2行 pool 両方枯渇 (2026-07-10)** — 残 = refuse-3行+ (~66 unit、--max-refusals 3 で選定) + DEFER cluster 群 (engine mini-wave 待ち。cluster 一覧 = DEFERRED-INDEX batch3/4/5 節末尾)。
- **★残枚数 随時報告義務 (2026-07-09 ユーザー指示)**: session 開始時 + batch/wave 出荷ごとに
  ...
  出荷後は本ファイルのこの行も更新する。
- vitest baseline = **4506 pass +1 skip** (batch3-5 probe +124) / smoke winsA=**472** exceptions=0 / 8 lint err0。
