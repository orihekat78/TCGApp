...
> - **次セッション = W6 後半 (step7-11 + card-phase 準備)**: step7 r70 証拠獲得 suppress (**gainSelfEvidence の async hirameki 越え defer 再順序化 — naive flag-gate 不可**) → step8 r75 reservedEffects queue (GameState 形状、smoke re-baseline 想定) → step9 r65 startContact 本実装 (flow-core、action:end emit !generatedByEffect guard = 唯一の Q&A pin) → step10 r9 leave:intercept pre-splice consult hook (AI-only 出荷、human-defender window は DEFER) → step11 findDeclaredAbility 5-site 共有 helper + removeAreaToDeckTop + hand-declared scope gate → **step12 card-phase 一括刈り取り** (解禁済: B09112/B09108/B09003/B06085/PR105/B08026/D10005/B05042/B07026/B09070/B09090*/B04072/B03046/B08014 + W1-W5 分)。spec = `.tmp/_w6_specs.json` rows 70/75/65/9/999 (着手時 origin/main 再 grep 必須)。
