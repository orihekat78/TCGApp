...
| `useDeclaredAbility` | `(state: GameState, uid: string, abilId: string, _ctx?: unknown): void` | useDeclaredAbility — 宣言能力使用を宣言する。 - declaredUseCount[abilId] をインクリメント - effect:declared を emit - ログ追加 cost 支払いは呼出元の responsibility (Phase 4 は分離). / |
