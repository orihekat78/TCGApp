# 🤖 Engine ハブ: engine.invariant

> ⚠️ このファイルは `scripts/gen-docs/gen-mapping.ts` により自動生成された。手で編集しない。
> 再生成: `npm run docs:mapping`
> Source hash: `054623c6171f`

`src/engine/invariant/` 配下のソースが参照している rules / specs / 関連 API リファレンスのハブ。

## 🔗 API リファレンス

- [`auto/api/invariant.md`](../../api/invariant.md)

## 📜 参照 Rule (6)

- [`01-victory-conditions.md`](../by-rule/01-victory-conditions.md)
- [`03-field-areas.md`](../by-rule/03-field-areas.md)
- [`06-card-types.md`](../by-rule/06-card-types.md)
- [`13-keywords.md`](../by-rule/13-keywords.md)
- [`20-color-and-switch.md`](../by-rule/20-color-and-switch.md)
- [`26-qa-deck-refresh.md`](../by-rule/26-qa-deck-refresh.md)

## 📄 ソース (9)

- [`src/engine/invariant/caseExists.ts`](../../../../src/engine/invariant/caseExists.ts)
- [`src/engine/invariant/caseMonotonic.ts`](../../../../src/engine/invariant/caseMonotonic.ts)
- [`src/engine/invariant/effectIsSerializable.ts`](../../../../src/engine/invariant/effectIsSerializable.ts)
- [`src/engine/invariant/frozenSurface.ts`](../../../../src/engine/invariant/frozenSurface.ts)
- [`src/engine/invariant/index.ts`](../../../../src/engine/invariant/index.ts)
- [`src/engine/invariant/partnerExists.ts`](../../../../src/engine/invariant/partnerExists.ts)
- [`src/engine/invariant/sceneAtMost5.ts`](../../../../src/engine/invariant/sceneAtMost5.ts)
- [`src/engine/invariant/scratchTraceMonotonic.ts`](../../../../src/engine/invariant/scratchTraceMonotonic.ts)
- [`src/engine/invariant/stunSemantics.ts`](../../../../src/engine/invariant/stunSemantics.ts)
