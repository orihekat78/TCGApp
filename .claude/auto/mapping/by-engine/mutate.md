# 🤖 Engine ハブ: engine.mutate

> ⚠️ このファイルは `scripts/gen-docs/gen-mapping.ts` により自動生成された。手で編集しない。
> 再生成: `npm run docs:mapping`
> Source hash: `02ff8e93bd70`

`src/engine/mutate/` 配下のソースが参照している rules / specs / 関連 API リファレンスのハブ。

## 🔗 API リファレンス

- [`auto/api/mutate.md`](../../api/mutate.md)

## 📜 参照 Rule (16)

- [`01-victory-conditions.md`](../by-rule/01-victory-conditions.md)
- [`03-field-areas.md`](../by-rule/03-field-areas.md)
- [`04-game-setup.md`](../by-rule/04-game-setup.md)
- [`05-turn-phases.md`](../by-rule/05-turn-phases.md)
- [`06-card-types.md`](../by-rule/06-card-types.md)
- [`09-cutin-disguise.md`](../by-rule/09-cutin-disguise.md)
- [`10-action-event.md`](../by-rule/10-action-event.md)
- [`11-reasoning.md`](../by-rule/11-reasoning.md)
- [`12-next-hint.md`](../by-rule/12-next-hint.md)
- [`13-keywords.md`](../by-rule/13-keywords.md)
- [`14-refresh.md`](../by-rule/14-refresh.md)
- [`16-card-set.md`](../by-rule/16-card-set.md)
- [`18-mr.md`](../by-rule/18-mr.md)
- [`19-special-rules.md`](../by-rule/19-special-rules.md)
- [`20-color-and-switch.md`](../by-rule/20-color-and-switch.md)
- [`26-qa-deck-refresh.md`](../by-rule/26-qa-deck-refresh.md)

## 📄 ソース (14)

- [`src/engine/mutate/case.ts`](../../../../src/engine/mutate/case.ts)
- [`src/engine/mutate/char.ts`](../../../../src/engine/mutate/char.ts)
- [`src/engine/mutate/deck.ts`](../../../../src/engine/mutate/deck.ts)
- [`src/engine/mutate/evidence.ts`](../../../../src/engine/mutate/evidence.ts)
- [`src/engine/mutate/file.ts`](../../../../src/engine/mutate/file.ts)
- [`src/engine/mutate/flag.ts`](../../../../src/engine/mutate/flag.ts)
- [`src/engine/mutate/gameResult.ts`](../../../../src/engine/mutate/gameResult.ts)
- [`src/engine/mutate/hand.ts`](../../../../src/engine/mutate/hand.ts)
- [`src/engine/mutate/index.ts`](../../../../src/engine/mutate/index.ts)
- [`src/engine/mutate/log.ts`](../../../../src/engine/mutate/log.ts)
- [`src/engine/mutate/partner.ts`](../../../../src/engine/mutate/partner.ts)
- [`src/engine/mutate/remove.ts`](../../../../src/engine/mutate/remove.ts)
- [`src/engine/mutate/scene.ts`](../../../../src/engine/mutate/scene.ts)
- [`src/engine/mutate/scratchTrace.ts`](../../../../src/engine/mutate/scratchTrace.ts)
