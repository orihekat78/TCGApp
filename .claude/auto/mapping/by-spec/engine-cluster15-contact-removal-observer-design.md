# 🤖 Spec 参照ハブ: engine-cluster15-contact-removal-observer-design

> ⚠️ このファイルは `scripts/gen-docs/gen-mapping.ts` により自動生成された。手で編集しない。
> 再生成: `npm run docs:mapping`
> Source hash: `0937356e9253`

仕様書 [`engine-cluster15-contact-removal-observer-design`](../../../specs/engine-cluster15-contact-removal-observer-design.md) を参照しているソース・関連 rule・関連 engine namespace のハブ。

## 📐 Spec 本体

- [`engine-cluster15-contact-removal-observer-design`](../../../specs/engine-cluster15-contact-removal-observer-design.md)

## 📜 関連 Rule

- [`07-action-flow.md`](../by-rule/07-action-flow.md)
- [`08-contact.md`](../by-rule/08-contact.md)
- [`13-keywords.md`](../by-rule/13-keywords.md)
- [`14-refresh.md`](../by-rule/14-refresh.md)
- [`15-abilities-effects.md`](../by-rule/15-abilities-effects.md)
- [`15-contact-removal-observer-design.md`](../by-rule/15-contact-removal-observer-design.md)
- [`17-icons.md`](../by-rule/17-icons.md)
- [`19-special-rules.md`](../by-rule/19-special-rules.md)
- [`20-color-and-switch.md`](../by-rule/20-color-and-switch.md)
- [`21-declared-ability-cost.md`](../by-rule/21-declared-ability-cost.md)
- [`24-qa-naming-stun.md`](../by-rule/24-qa-naming-stun.md)

## 📄 参照ソース (3)

- [`src/cards/ct-d10/D10007.ts`](../../../../src/cards/ct-d10/D10007.ts)
- [`src/cards/ct-d10/D10008.ts`](../../../../src/cards/ct-d10/D10008.ts)
- [`src/cards/ct-p09/B09026.ts`](../../../../src/cards/ct-p09/B09026.ts)
