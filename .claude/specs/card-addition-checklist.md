...
- [ ] **画面処理がカードテキスト文言と一致** (2026-06-06 追加、BUG-117〜121 教訓): 実機 (Playwright) で
      候補列挙 / 選択 / 結果 (盤面・手札・証拠) が、カードの **公式テキストの文言**と一字一句の観点で
      齟齬しないか確認する。特に以下を **「テキストの語」と「画面の挙動」を 1 対 1 で突き合わせる**:
  - **対象範囲** (「相手の」「自分の」「現場の」) = 画面で正しい side/area の候補のみ出るか
  - **filter 条件** (「LP0の」「レベル7以下の」「【青】の」「特徴[警察]の」「イベント」「スリープ状態の」)
    = 条件外のカードが候補に**混入しない / 条件内が漏れない** か (decoy を盤面に置いて確認)
    - **アイコン能力 filter** (「カットインを持つキャラ」等): `filter.keyword:'カットイン'|'変装'|'ヒラメキ'|'ミスリード'`
      は keywords[] に無く ability 構造で表現される → `engine.read.keyword.defHasKeyword` が検出 (BUG-122)
    - **エリア × kind**: イベントを含むエリア (hand/remove/deck) から「**キャラ**」を選ぶ pick は
      `kind:'character'` 必須 (色/レベルのみだと同条件イベントが混入、BUG-123)。scene area は char 専用で不要
  - **枚数** (「1枚まで」「2枚」「すべて」) と **選択者** (「効果を使うプレイヤーが選ぶ」= controller)
  - **持続** (「ターン終了時まで」) = ターンを跨いで解除されるか
  - **複数択** (「以下から1つ選んで行う」) = 画面に選択 modal が出て human が選べるか (option 0 既定化しない)
  - 「画面表示確認 ≠ テキスト一致確認」。表示が出るだけでなく **テキストの語義通りの分岐**を実機で踏むこと
