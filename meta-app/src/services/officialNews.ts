...
function officialUrl(value: string): string | null {
  try {
    ...
    if (url.protocol !== "https:" || url.hostname !== "www.takaratomy.co.jp")
      return null;
...
function readCache(storage: LoadOptions["storage"]): NewsCache | null {
  ...
  try {
    ...
    return items.length
      ? { version: CACHE_VERSION, fetchedAt: value.fetchedAt!, items }
      : null;
  } catch {
    return null;
  }
...
export async function loadOfficialNews({
  ...
  try {
    const response = await fetcher(OFFICIAL_NEWS_URL, {
      referrerPolicy: "no-referrer",
      signal: controller.signal,
    });
    if (!response.ok)
      throw new Error(`Official NEWS request failed: ${response.status}`);
    const items = parseOfficialNews(await response.text());
    if (!items.length)
      throw new Error("Official NEWS markup did not contain valid entries");
    const next = {
