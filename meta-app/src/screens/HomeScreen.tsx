          ...
          {deck && partner && incident ? (
            <>
              <div className="home-deck-heading">
                <h1 id="home-deck-name" title={deck.name}>
                  {deck.name}
                </h1>
                <button
                  ref={changeDeckRef}
            ...
            <div className="home-empty-deck">
              ...
              <button type="button" onClick={() => navigate("deck")}>
                デッキを作成
              </button>
            </div>
          )}
...
function HomeLegalNotice() {
  return (
    <footer
      className="home-legal-notice"
      data-testid="home-legal-notice"
      aria-label="利用上の注意"
    >
      <p>This is an unofficial fan project for personal use only.</p>
      <p>© 青山剛昌／小学館 © TOMY</p>
      <p>公式商品ではありません。</p>
    </footer>
  );
...
function IdentityCard({
  ...
  return (
    <figure
      className={`home-identity-card home-identity-card--${role}`}
...
function OfficialNewsSection({
  result,
}: {
  result: ReturnType<typeof useOfficialNews>;
}) {
  return (
    <section className="home-rail-section" aria-labelledby="home-news-title">
      <div className="home-section-heading">
        ...
        <a
          href="https://www.takaratomy.co.jp/products/conan-cardgame/"
          target="_blank"
      ...
      {result.items.length ? (
        ...
        <p className="home-rail-empty">
          公式NEWSを読み込めませんでした。公式サイトで最新情報をご確認ください。
        </p>
      )}
      {result.source === "stale" && (
        <p className="home-sync-note">前回取得した情報を表示中</p>
      )}
    </section>
  );
}
        ...
        <ul className="home-match-list">
          {history.map((match) => {
            ...
            return (
              <li key={match.id}>
                <span
                  className={`home-match-result ${observed ? "is-observe" : match.won ? "is-win" : "is-loss"}`}
                >
                  {observed
                    ? `CPU ${match.won ? "1" : "2"}勝利`
                    : match.won
                      ? "勝利"
                      : "敗北"}
                </span>
                <span title={match.oppDeckName ?? "CPU"}>
                  {match.oppDeckName ?? "CPU"}
                </span>
                {recorded ? (
